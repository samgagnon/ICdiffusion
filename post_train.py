...
parser.add_argument('--nf', default=None, type=float, help='neutral fraction')
...
gfile_stream = open(os.path.join(config.model.workdir, f'stdout_{num_bins}_post.txt'), 'w')
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, optimizer, ema, epoch):
        ...
        torch.save(
            ...
            os.path.join(self.checkpoint_path, f'best_sgd_checkpoint_{nf}.pth')
            )
...
checkpoint_path = os.path.join(checkpoint_dir, f'best_sgd_checkpoint_{nf}.pth')
...
training_dataset = GalaxyDataset(datadir=scratch_ddir, job_type=args.j, train_or_val='training', single_nf=nf)
training_loader = DataLoader(training_dataset, config.training.batch_size, shuffle=True, num_workers=args.num_workers)
validation_dataset = GalaxyDataset(datadir=scratch_ddir, job_type='galsmear', train_or_val='validation', single_nf=nf)
...
for epoch in range(init_epoch, config.training.n_epochs + 1):
    ...
    if epoch % 10 == 0:
        ...
        torch.save(
            dict(optimizer=optimizer.state_dict(), model=model.module.state_dict(), ema=ema.state_dict(), epoch=epoch),
            os.path.join(checkpoint_dir, f'checkpoint_{epoch}_post.pth')
            )
    torch.save(
        dict(optimizer=optimizer.state_dict(), model=model.module.state_dict(), ema=ema.state_dict(), epoch=epoch),
        os.path.join(checkpoint_dir, f'checkpoint_post.pth')
        )
